...
# Data loading
tr <- read_csv("derived_data/train_imputed.csv", col_types = cols(Alpha = col_factor()));
te <- read_csv("derived_data/test_imputed.csv");
gr <- read_csv("source_data/greeks.csv", col_types = cols(Alpha = col_factor()));
tr <- inner_join(dplyr::select(tr,-Class),dplyr::select(gr,c(Id,Alpha)));
...
for (split in vfold_cv(tr, nfolds)$splits) {
  ...
  y_val <- as.character(X_val$Alpha) %>% as.integer()
  X_val <- dplyr::select(X_val, -Alpha) %>% data.matrix()
  ...
  for (seed in seq(nbags)) {
    ...
    X <- rec %>%
      step_downsample(Alpha, seed = seed, skip = FALSE) %>% 
      prep() %>%
      bake(new_data = tr[tri, ]) 
    y <- as.character(X$Alpha) %>% as.integer()
    X <- dplyr::select(X, -Alpha) %>% data.matrix()
    
    m_lgb <- lgb.train(params = list(objective = "multiclass",
                                     num_class = 4,
                                     nthread = 4,
                                     eta = 0.0025,
                                     num_leaves = 5, 
...
submission <- read_csv("source_data/sample_submission.csv") %>% 
  mutate(class_0 = pred[,1]) %>%
  mutate(class_1 = 1 - class_0);
