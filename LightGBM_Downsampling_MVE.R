...
# Data loading
tr <- read_csv("derived_data/train_imputed.csv", col_types = cols(Class = col_factor()));
...
for (split in vfold_cv(tr, nfolds)$splits) {
  ...
  for (seed in seq(nbags)) {
    ...
    oof[-tri] <- oof[-tri] + predict(m_lgb, X_val) / nbags
    p <- p + predict(m_lgb, X_te) / nbags
  }
  
  scores <- c(scores, balanced_ll(oof[-tri], y_val))  
  pred <- pred + p / nfolds
}

cat("Mean score:", mean(scores), "\n")

tibble(Y_true = as.integer(as.character(tr$Class)), 
       OOF = round(oof)) %>%
  pivot_longer(cols = where(is.numeric),
               names_to = "Values",
               values_to = "Class",
               values_drop_na = TRUE) %>%
  ggplot(aes(y = factor(Class), fill = Values)) +
...
tibble(Y_true = as.integer(as.character(tr$Class)), 
  ...
  pivot_longer(cols = where(is.numeric),
               ...
               values_to = "Class",
               values_drop_na = TRUE) %>%
  ggplot(aes(x = Class, fill = Values)) +
